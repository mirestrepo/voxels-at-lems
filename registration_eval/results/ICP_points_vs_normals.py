...
def main(argv=None):
    ...
    ICN_error_mean= np.zeros((len(niter), 2));
    ICN_error_min= np.zeros((len(niter), 2))
    ICN_error_max= np.zeros((len(niter), 2))
    ICN_error_median= np.zeros((len(niter), 2))
    ...
    for d_idx in range(0, len(descriptors)):
      ...
      for iter_idx in range(0,len(niter)):
        ...
        ICN_error = np.zeros((ntrials, 2));
        ICP_error = np.zeros((ntrials, 2));

        for trial in range(0,ntrials):
          ICP_error[trial,:], ICN_error[trial,:] = compute_error(trial, descriptor, niter[iter_idx], 99)
      
        #Compute mean, max and min
        ICN_error_mean[iter_idx, :] = np.mean(ICN_error, axis=0)
        ICP_error_mean[iter_idx, :]= np.mean(ICP_error, axis=0)
        ICN_error_max[iter_idx, :] = np.max(ICN_error, axis=0)
        ICP_error_max[iter_idx, :]= np.max(ICP_error, axis=0)
        ICN_error_min[iter_idx, :] = np.min(ICN_error, axis=0)
        ICP_error_min[iter_idx, :]= np.min(ICP_error, axis=0)
        ICN_error_median[iter_idx, :] = np.median(ICN_error, axis=0)
        ICP_error_median[iter_idx, :]= np.median(ICP_error, axis=0)
      # import code; code.interact(local=locals())
      
    
      # import code; code.interact(local=locals())
        # ICN_error_mean[iter_idx, :] =   
    
      if (plot_errors):
            
        #plot IA, ICP --> missing to to ICP_normals
        axT.errorbar(niter, ICN_error_median[:, 1], yerr=[ ICN_error_median[:, 1]-ICN_error_min[:, 1], ICN_error_max[:, 1]- ICN_error_median[:, 1]], fmt=markers[2*d_idx], color=colors[2*d_idx], label=descriptor + " IA + ICPN", capsize=12);
        axT.errorbar(niter, ICP_error_median[:, 1], yerr=[ICP_error_min[:, 1], ICP_error_max[:, 1]], fmt=markers[2*d_idx+1], color=colors[2*d_idx+1], label=descriptor + " IA+ICP", capsize=12, ms=14)
        
        axR.errorbar(niter, ICN_error_median[:, 0], yerr=[ICN_error_median[:, 0]- ICN_error_min[:, 0], ICN_error_max[:, 0]-ICN_error_median[:, 0]], fmt=markers[2*d_idx], color=colors[2*d_idx], label=descriptor + " IA + ICPN", capsize=12);
        axR.errorbar(niter, ICP_error_median[:, 0], yerr=[ICP_error_median[:, 0] - ICP_error_min[:, 0], ICP_error_max[:, 0]- ICP_error_median[:, 0]], fmt=markers[2*d_idx+1], color=colors[2*d_idx+1], label=descriptor + " IA+ICP", capsize=12, ms=14)
     
        #********************Detail Plot***************************
        axT_detail.plot(niter, ICN_error_median[:, 1], markers[2*d_idx], color=colors[2*d_idx], label=descriptor + " IA+ICPN");
        axT_detail.plot(niter, ICP_error_median[:, 1], markers[2*d_idx+1], color=colors[2*d_idx+1], label=descriptor + " IA+ICP", ms=14)
        
        axR_detail.plot(niter, ICN_error_median[:, 0], markers[2*d_idx], color=colors[2*d_idx], label=descriptor + " IA+ICPN");
        axR_detail.plot(niter, ICP_error_median[:, 0], markers[2*d_idx+1], color=colors[2*d_idx+1], label=descriptor + " IA+ICP", ms=14)
    ...
    if (plot_errors):  
      ...
      figT.savefig("/Users/isa/Experiments/reg3d_eval/downtown_dan/T_error_icn.pdf", transparent=True, pad_inches=5)
      figT.savefig("/Users/isa/Experiments/reg3d_eval/downtown_dan/T_error_icn.eps", transparent=True, pad_inches=5)
      ...
      figR.savefig("/Users/isa/Experiments/reg3d_eval/downtown_dan/R_error_icn.pdf", transparent=True)
      
      
      #********************Detail Plot***************************
      axT_detail.set_xlim((90,210) ); 
      axT_detail.set_xticks((100,200 ) );
      axT_detail.set_yticks(np.arange(0,6,1));
      axT_detail.set_ylim((0,6))
      figT_detail.savefig("/Users/isa/Experiments/reg3d_eval/downtown_dan/T_detail_error_icn.pdf", transparent=True)


      axR_detail.set_xlim((90,210) );
      axR_detail.set_xticks((100,200 ) );
      axR_detail.set_yticks(np.arange(0.0, np.pi/360 , np.pi/(180*8)));
      axR_detail.set_yticklabels(np.arange(0, 0.5, 0.125))
      axR_detail.set_ylim((0,np.pi/(180*2)))
      figR_detail.savefig("/Users/isa/Experiments/reg3d_eval/downtown_dan/R_detail_error_icn.pdf", transparent=True)
