...
def main(argv=None):
    ...
    for d_idx in range(0, len(descriptors)):
      ...
      if (plot_errors):
        ...
        axT.errorbar(niter, IA_error_median[:, 1], yerr=[ IA_error_median[:, 1]-IA_error_min[:, 1], IA_error_max[:, 1]- IA_error_median[:, 1]], fmt=markers[2*d_idx], color=colors[2*d_idx], label=descriptor + " IA", capsize=12);
        ...
        axR.errorbar(niter, IA_error_median[:, 0], yerr=[IA_error_median[:, 0]- IA_error_min[:, 0], IA_error_max[:, 0]-IA_error_median[:, 0]], fmt=markers[2*d_idx], color=colors[2*d_idx], label=descriptor + " IA", capsize=12);
        ...
        #********************Detail Plot***************************
        axT_detail.plot(niter, IA_error_median[:, 1], markers[2*d_idx], color=colors[2*d_idx], label=descriptor + " IA");
        axT_detail.plot(niter, ICP_error_median[:, 1], markers[2*d_idx+1], color=colors[2*d_idx+1], label=descriptor + " IA+ICP", ms=14)
        
        axR_detail.plot(niter, IA_error_median[:, 0], markers[2*d_idx], color=colors[2*d_idx], label=descriptor + " IA");
    ...
    if (plot_errors):  
      ...
      figR.savefig("/Users/isa/Experiments/reg3d_eval/downtown_dan/R_error.pdf", transparent=True)
