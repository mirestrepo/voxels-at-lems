...
def random_rot_angle(angle_max):
    """ Generate a random rotation angle in S^3 with a cuffoff angle of angle_max using rejection sampling.
    See http://math.stackexchange.com/questions/131336/uniform-random-quaternion-in-a-restricted-angle-range
    for a discussion"

    @param angle: The output random angle
    @param angle_max: The cutoff value
    """
    reject_angle = True
    while reject_angle :
        # Random angle in [0, angle_max^3]
        u = angle_max * angle_max * angle_max * uniform(0,1)
        alpha = u ** (1.0/3.0)
        t = uniform(0,1)

        # print t , alpha
        # print sin(alpha / (alpha * alpha)) * sin(alpha/ (alpha * alpha))

        if  t < (sin(alpha / (alpha * alpha)) * sin(alpha/ (alpha * alpha))) :
            angle = alpha
            reject_angle = False
            print angle
