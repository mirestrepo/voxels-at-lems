...
class bof_job():
    def __init__(self, categories, codebook , scene_id, path_out):
...
class bof_worker(multiprocessing.Process):
    ...
    def run(self):
        while not self.kill_received:
            ...
            dbrec3d_batch.init_process("bof_learn_categories_ply_process");
            dbrec3d_batch.set_input_from_db(0, job.codebook);
            dbrec3d_batch.set_input_int(1, job.scene_id);
            dbrec3d_batch.set_input_from_db(2, job.categories);
...
if __name__=="__main__":
  ...
  for scene_id in range (0, nscenes):
      ...
      current_job = bof_job(categories, codebook , scene_id, histogram_path);
