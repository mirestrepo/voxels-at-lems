...
#################################################################################################################################
main_dir="/Users/isa/Experiments/BOF/helicopter_providence/pmvs_ssd";
#################################################################################################################################
...
if  $learn_codebook; then 
  num_cores=$nclasses;
  T1=$(date +%s)
  python2.7 /Projects/voxels-at-lems/scripts/dbrec3d/bof/general/learn_category_codebook/learn_category_codebook_from_ply.py --bof_dir $bof_dir --k_means_dir $k_means_dir --num_cores $num_cores --num_means $num_means --fraction $fraction --J $J --max_it $max_it --nclasses $nclasses
  DIFF=$(( $(date +%s) - $T1 ))
...
if $learn_categories; then
  num_cores=8; 
  T=$(date +%s) 
  python2.7 /Projects/voxels-at-lems/scripts/dbrec3d/bof/general/learn_categories/learn_categories_from_ply.py --bof_dir $bof_dir --k_means_dir $k_means_dir --class_histograms_dir $class_histograms_dir --num_cores $num_cores
  DIFF=$(( $(date +%s) - $T ))
...
if $classify; then
  num_cores=8;
  T=$(date +%s) 
  python2.7 /Projects/voxels-at-lems/scripts/dbrec3d/bof/general/classify/classify_ply.py --bof_dir $bof_dir --k_means_dir $k_means_dir --class_histograms_dir $class_histograms_dir --classification_dir $classification_dir --num_cores $num_cores --ncategories $ncategories
  DIFF=$(( $(date +%s) - $T ))
