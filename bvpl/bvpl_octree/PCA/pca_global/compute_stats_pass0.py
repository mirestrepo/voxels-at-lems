...
class pca_stats_job():
    def __init__(self, global_pca, scene_id, block_i, block_j, block_k, file_out):
...
def execute_pca_jobs(jobs, num_procs=4):
    ...
    for i in range(num_procs):
        ...
        time.sleep(0.001);  #This is needed because when random sampling is done the current time is taken as the seed
      ...
      for block_i in range (0, nblocks[1]):
          for block_j in range (0, nblocks[2]):
              for block_k in range (0, nblocks[3]):
                  ...
                  current_job = pca_stats_job(global_pca, s_idx, block_i, block_j, block_k, file_out);
                  job_list.append(current_job);
                  i= i+1;          
                              
  results = execute_pca_jobs(job_list, num_cores);
  print ("Pass 0 done")
