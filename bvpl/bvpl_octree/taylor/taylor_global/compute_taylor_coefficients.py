...
Computes taylor coefficients. Each block is processed in a separate thread.
"""
import os;
...
class taylor_project_job():
    def __init__(self, taylor_path, scene_id, block_i, block_j, block_k, dimension):
...
if __name__=="__main__":
  ...
  print("**********Executing: compute_taylor_coefficients.py **********")
      ...
      for block_i in range (0, nblocks[1]):
          for block_j in range (0, nblocks[2]):
              for block_k in range (0, nblocks[3]):
                  current_job = taylor_project_job(taylor_dir, s_idx, block_i, block_j, block_k, dimension);
